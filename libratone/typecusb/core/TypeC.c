...
void StateMachineTypeC(void)
{
    ...
    do {
        ...
        if(utccDefault == SinkCurrent){
            platform_delay_10us(10000);
        }
...
{
    platform_notify_attached(false);
#ifdef FSC_INTERRUPT_TRIGGERED
    g_Idle = TRUE;
    platform_enable_timer(FALSE);
#endif // FSC_INTERRUPT_TRIGGERED
...
{
    USBTypeCCurrent cur_current = utccNone;

    /* BC_LVL is only valid if there is no PD Activity */
    if (Registers.Status.ACTIVITY == 0) {
        switch (Registers.Status.BC_LVL) {
        case 0b01:                       // If we detect the default...
            cur_current  = utccDefault;
            break;

        case 0b10:                       // If we detect 1.5A
            cur_current = utcc1p5A;
            break;

        case 0b11:                       // If we detect 3.0A
            cur_current = utcc3p0A;
            break;

        default:
            cur_current = utccNone;
            break;
        }

        if (AttachedSink != ConnState) {
            cur_current = utccNone;
        }

        if (SinkCurrent != cur_current) {
            SinkCurrent = cur_current;
            platform_notify_sink_def_current(SinkCurrent);
        }
    }
}
#endif // FSC_HAVE_SNK
...
void SetStateIllegalCable(void)
{
    ...
    Registers.Measure.MDAC = MDAC_0P882;
    DeviceWrite(regMeasure, 1, &Registers.Measure.byte);           // Commit the DAC threshold
