...
doDataObject_t                  SinkRequestPre = {0};                            // Current USB PD contract (request object)
...
void PolicySinkTransitionDefault(void)
{
    ...
    case 2:
        if (isVBUSOverVoltage(VBUS_MDAC_4P20)) {
            PolicySubIndex++;
#ifdef FSC_DEBUG
            debugPrint("isVBUSOverVoltage\n");
#endif
        } else if (platform_check_timer(&PolicyStateTimer)) {                                  // Break out if we never see 0V
#ifdef FSC_DEBUG
            debugPrint("tSrcRecoverMax timeout\n");
#endif
...
{
    static FSC_BOOL first_no_resp = TRUE;
    ...
    if (ProtocolMsgRx) {                                                        // If we have received a message...
        ...
        if (platform_check_timer(&PolicyStateTimer)) {
#ifdef FSC_DEBUG
            debugPrint("PolicyStateTimer timeout, HardResetCounter=%d\n", HardResetCounter);
#endif
            PolicyState = peSinkSendHardReset;
            PolicySubIndex = 0;
        }
    } else {
        if (platform_check_timer(&NoResponseTimer)) {
            if (PolicyHasContract == TRUE) {
                PolicyState = peErrorRecovery;
            } else {
#ifdef FSC_INTERRUPT_TRIGGERED
                ...
                if (first_no_resp) {
                    platform_notify_pd_sel_cap(0, 0);
                    first_no_resp = FALSE;
                }
...
void PolicySinkEvaluateCaps(void)
{
...
#ifdef FSC_DEBUG
        debugPrint("SinkRequestMaxVoltage=%d, CapsSink[%d].FPDOSink.Voltage=%d\n", \
                   SinkRequestMaxVoltage, i, CapsSink[i].FPDOSink.Voltage);
#endif
    ...
    for (i = 0; i < CapsHeaderReceived.NumDataObjects; i++) {                   // Going to select the highest power object that we are compatible with
        ...
        case pdoTypeFixed:
#ifdef FSC_DEBUG
            debugPrint("caps[%d].SupplyType=pdoTypeFixed, Voltage=%d, MaxCurrent=%d\n", \
                       i, CapsReceived[i].FPDOSupply.Voltage, \
...
#ifdef FSC_DEBUG
            debugPrint("Caps[%d].SupplyType=pdoTypeVariable, MaxVoltage=%d, MinVoltage=%d, MaxCurrent=%d\n", \
                       i, CapsReceived[i].VPDO.MaxVoltage, CapsReceived[i].VPDO.MinVoltage, \
                       CapsReceived[i].VPDO.MaxCurrent);
#endif
    ...
    if ((reqPos > 0) && (SelVoltage > 0)) {
        ...
        SinkRequestPre.object = 0;
        SinkRequestPre.FPDOSupply.Voltage = SelVoltage;
        SinkRequestPre.FPDOSupply.MaxCurrent = SinkRequest.FVRDO.OpCurrent;
    ...
    if (ProtocolMsgRx) {
        ...
        if (PolicyRxHeader.NumDataObjects == 0) {                               // If we have received a control message...
            ...
            case CMTPS_RDY:
                ...
                platform_notify_pd_contract(TRUE);
                platform_notify_pd_sel_cap(SinkRequestPre.FPDOSupply.Voltage * 50, \
                                           SinkRequestPre.FPDOSupply.MaxCurrent * 100);
                break;
