...
void USBPDProtocol(void)
{
    ...
    if (Registers.Status.I_HARDRST /*&& Registers.Status.HARDRST*/) {
        ResetProtocolLayer(TRUE);                                               // Reset the protocol layer

        if (PolicyIsSource) {                                                   // If we are the source...
            platform_set_timer(&PolicyStateTimer, tPSHardReset);
        ...
        if (lastState != ProtocolState) {
            debugPrint("ProtocolState=%d\n", ProtocolState);
        }

#endif
        lastState = ProtocolState;
...
#ifdef FSC_DEBUG
    debugPrint("NumDataObjects=%d, MessageType=%d, MessageID=%d, PolicyRxHeader.MessageID=%d\n", \
               PolicyRxHeader.NumDataObjects, PolicyRxHeader.MessageType, MessageID, PolicyRxHeader.MessageID);
#endif
...
void ProtocolTransmitMessage(void)
{
    ...
    platform_set_timer(&ProtocolTimer, 0); /* 3ms */
...
void ProtocolSendHardReset(void)
{
    FSC_U8 data;
#ifdef FSC_DEBUG
    debugPrint("send the hard reset\n");
#endif
...
void manualRetries(void)
{
    ...
    if (Registers.Status.I_GCRCSENT) {
        ...
        ProtocolGetRxPacket();
        ProtocolState = PRLIdle;                                        // Set the idle state
        PDTxStatus = txCollision;                                           // Set the transmission status to error to signal the policy engine
    }
    ...
    if (Registers.Status.I_TXSENT) {
        ...
        ProtocolVerifyGoodCRC();
    } else if (platform_check_timer(&ProtocolTimer) && !g_retryfail_lock) { /* Also set on collision */
        ...
        if (!g_tries) {                                                       // If no more retries, set as failure
            ...
            if (g_collision_lock) {
                DeviceWrite(regFIFO, ProtocolTxBytes, &ProtocolTxBuffer[0]);        // Reload FIFO                                                       // Decrement Retries if there is no collision
            }

            /* May need to flush a late GoodCRC in 302 */
